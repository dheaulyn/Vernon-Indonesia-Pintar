...
class _ProgramDetailAdminState extends State<ProgramDetailAdmin> with SingleTickerProviderStateMixin {
  ...
  void _loadData() {
    ...
    if (data['requirements'] != null) {
      for (var req in data['requirements']) {
        ...
        _reqControllers.add(ReqController(TextEditingController(text: req['title']), pts));
      }
    }
    ...
    if (data['timeline'] != null) {
      for (var step in data['timeline']) {
        _timelineControllers.add(TimelineController(
  ...
  void _saveData() {
    ...
    List<Map<String, dynamic>> updatedReqs = _reqControllers.map((reqCtrl) {
      return {
        ...
        "points": reqCtrl.pointsCtrl.map((p) => p.text).where((text) => text.isNotEmpty).toList(),
      };
    }).toList();

    // Kumpulkan data Timeline
    List<Map<String, String>> updatedTimeline = _timelineControllers.map((timeCtrl) {
      return {
    ...
    _showSuccessMessage('Detail Program berhasil diperbarui!');
  }
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              const Text('Kelola Detail Program', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
              ElevatedButton.icon(
                onPressed: _saveData,
          ...
          if (index == _reqControllers.length) {
            return Center(
              child: ElevatedButton.icon(
                onPressed: () => setState(() => _reqControllers.add(ReqController(TextEditingController(), [TextEditingController()]))),
                icon: const Icon(Icons.add, color: Colors.white),
                label: const Text("Tambah Kategori Syarat Baru", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
          ...
          return Container(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Expanded(child: TextField(controller: req.titleCtrl, decoration: const InputDecoration(labelText: "Judul Kategori (Cth: Syarat Umum)", border: OutlineInputBorder()))),
                    IconButton(
                ...
                const Text("Poin Persyaratan:", style: TextStyle(fontWeight: FontWeight.bold)),
                  ...
                  return Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(child: TextField(controller: req.pointsCtrl[pIndex], decoration: const InputDecoration(isDense: true, border: OutlineInputBorder()))),
                        IconButton(
                ...
                TextButton.icon(
                  onPressed: () => setState(() => req.pointsCtrl.add(TextEditingController())),
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          if (index == _timelineControllers.length) {
            return Center(
              child: ElevatedButton.icon(
                onPressed: () => setState(() => _timelineControllers.add(TimelineController(TextEditingController(), TextEditingController()))),
                icon: const Icon(Icons.add, color: Colors.white),
                label: const Text("Tambah Langkah Alur", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, shape: const StadiumBorder()),
              ),
            );
          }
          ...
          return Container(
            ...
            child: Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                CircleAvatar(backgroundColor: AppColors.primary, child: Text("${index + 1}", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
                const SizedBox(width: 15),
                Expanded(
                  child: Column(
                    children: [
                      TextField(controller: step.titleCtrl, decoration: const InputDecoration(labelText: "Judul Langkah", isDense: true, border: OutlineInputBorder())),
                      const SizedBox(height: 10),
                      TextField(controller: step.descCtrl, maxLines: 2, decoration: const InputDecoration(labelText: "Deskripsi", isDense: true, border: OutlineInputBorder())),
                    ],
                  ),
                ),
                IconButton(
